...
class Turn:
    ...
    def annotate_raw(cls, raw):
        ...
        fix = {'centre': 'center', 'areas': 'area', 'phone number': 'phone'}
        return cls(
    ...
    def numericalize_(self, vocab):
        ...
        self.num['system_acts'] = [vocab.word2index(['sos'] + [w.lower() for w in a] + ['eos'], train=True) for a in self.system_acts + [['eos']]]
...
class Dataset:
    ...
    def batch(self, batch_size, shuffle=False):
        ...
        for i in tqdm(range(0, len(turns), batch_size), disable=True):
    ...
    def evaluate_preds(self, preds):
        ...
        for d in self.dialogues:
            ...
            for t in d.turns:
                ...
                i += 1
        # print('joint_goal: {:.4f}'.format(np.mean(joint_goal)))
        return {'turn_inform': np.mean(inform), 'turn_request': np.mean(request), 'joint_goal': np.mean(joint_goal)}

    def record_preds(self, preds, to_file, wall_time=1.0):
        data = self.to_dict()
        i = 0
